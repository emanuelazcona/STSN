...
def weightCreation(start, end, wN, timeN):	# create arrays for frozen and trainable weights
	...
	if start <= end:
		...
		if start > 1:
			...
			wL = tf.ones(shape = [timeN, start-1], dtype = tf.float64)
		...
		if end < wN:
			...
			wR = tf.ones(shape = [timeN, wN - end], dtype = tf.float64)
		...
		wT = tf.Variable(tf.ones(shape = [timeN, end - start + 1], dtype = tf.float64))
