...
least_squares = tf.norm(Yhat_masked, ord=2)**2
print("Done!\n")
...
with tf.Session(config=tf.ConfigProto(gpu_options=tf.GPUOptions(allow_growth=True))) as sess:
	...
	for i in range(1, epochs+1):
		...
		print("Epoch: " + str(i) + "\t\tLoss: " + str(loss_value) + "\t\tScatter: " + userScatter + "\t\tTime: " + userTime)

		firstPeak = (layers//2)//2
		print("Peek Weights @ Time Unit " + str(firstPeak) + ": ")
		print(W_tens[firstPeak,:].eval())

		secPeak = (layers+ layers//2)//2
		print("\nPeek Weights @ Time Unit " + str(secPeak + 1) + ": ")
		print(W_tens[secPeak,:].eval())
