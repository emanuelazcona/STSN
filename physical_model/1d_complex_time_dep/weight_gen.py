...
def create_weights(start, end, wN, time):	# create arrays for frozen and trainable weights
	...
	if start <= end:
		...
		if start > 1:
			...
			wL = [tf.zeros(shape = [1, start-1], dtype = tf.float64) for _ in range(time)]
		...
		if end < wN:
			...
			wR = [tf.zeros(shape = [1, wN - end], dtype = tf.float64) for _ in range(time)]
		...
		wT = [tf.Variable(tf.zeros(shape = [1, end - start + 1], dtype = tf.float64)) for _ in range(time)]
...
def concat_weights(wL, wT, wR):
	...
	if wL is None:
		...
		# i.e.: w = [#, #, #, #, 1, 1] where # is a trainable weight and 1 is the constant untrainable free space
			...
			w = [tf.concat([t, r], axis = 1) for t,r in zip(wT,wR)]
	...
	# masking does not start at first spatial weight
		...
		if wR is None:
			w = [tf.concat([l, t], axis = 1) for l,t in zip(wL,wT)]

		# masking is somewhere between the first and last spatial weights
		else:
			w = [tf.concat([l,t,r], axis = 1) for l,t,r in zip(wL,wT,wR)]
